...
POWER_SCRIPT = BIN_DIR / f"{APP_ID}-power"                # Ctrl+Alt+Delete: the session menu
...
def power_script_text(port: int) -> str:
    """Ctrl+Alt+Delete, in its own script.

    Inline was tried and does not survive: the JSON body needs double quotes, the
    sway `exec` line wraps it in `sh -c '...'`, and sway's own parser eats commas
    and semicolons before the shell ever sees them. idle_script_text exists for
    exactly the same reason.
    """
    return f"""\
#!/bin/sh
# AgentOS session menu (Ctrl+Alt+Delete) — generated by `agentos install-session`.
#
# Bound so the COMPOSITOR claims this key. Unbound, the kernel's VT handler
# answers it and ctrl-alt-del.target is an alias for reboot.target: the machine
# reboots instantly, nothing saved, nothing asked. Every other desktop claims it.
#
# First choice is the AgentOS session menu, which offers lock / log out / restart
# / shut down. If the desktop is not answering — which is exactly when you reach
# for this key — the compositor puts the same choices on screen itself, so
# something always appears and nothing happens that you did not pick.
if curl -sf -m 2 -X POST -H 'Content-Type: application/json' \\
     -d '{{"action":"power"}}' \\
     "http://127.0.0.1:{port}/api/shell/action" >/dev/null 2>&1; then
  exit 0
fi
exec swaynag -t warning \\
  -m 'AgentOS is not responding. What would you like to do?' \\
  -B 'Log out' 'swaymsg exit' \\
  -B 'Restart the desktop' 'systemctl --user restart agentos' \\
  -Z 'Cancel' 'true'
"""
...
# ---------------------------------------------------------------------------
# Ctrl+Alt+Delete. This MUST be bound, and the reason is not convenience.
#
# Unbound, the compositor does not claim the key, and on a Linux VT the kernel's
# own handler is what answers it: ctrl-alt-del.target is an alias for
# reboot.target, so the machine reboots instantly with nothing saved and no
# prompt. That is what "I press Ctrl+Alt+Delete and it completely loses it" was.
# Every other desktop claims this key; ours was the only one that did not.
#
# Bound, it does what people expect from it — offers a CHOICE. A key you reach for
# when something is already wrong must never be the key that throws your session
# away without asking.
#
# The fallback matters as much as the binding: the moment you most need this is
# the moment the shell may not be answering, so if the desktop does not respond
# the compositor puts the choices on screen itself with swaynag. Either way
# something appears, and nothing happens that you did not pick.
bindsym Ctrl+Alt+Delete exec "{POWER_SCRIPT}"

# The emergency hatch, kept separate and kept immediate: this one does not ask,
# because it exists for when nothing can ask. Ctrl+Alt+F3 for a raw TTY is the
# kernel's, not ours.
...
def stage(wayland: bool = True, port: int | None = None) -> list[Path]:
    ...
    if wayland:
        ...
        POWER_SCRIPT.write_text(power_script_text(port))
        POWER_SCRIPT.chmod(0o755)
        written.append(POWER_SCRIPT)
...
def remove(autologin: bool = False):
    if not IS_LINUX:
        ...
        return
    for p in (SESSION_SCRIPT, SHELL_SCRIPT, X11_SESSION_SCRIPT, POWER_SCRIPT, SWAY_CONF):
        if p.exists():
            p.unlink()
            print(f"✓ removed {p}")
