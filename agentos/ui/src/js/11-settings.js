...
function setTab(body,all){
  ...
  if(want('agent')){
    ...
    P.push(pGroup('Sandbox',[
      ...
      pRow('Safe folders',pArea('s-sb-folders',((cfg.sandbox&&cfg.sandbox.folders)||[]).join('\n'),'/data/reports\n/srv/shared'),
        {stack:true,desc:'Extra folders the agent and the Terminal may read and write, one per line. Everything else stays read-only. Folders holding other accounts are refused.',f:'sandbox safe folders extra data access'}),
    ],{f:'sandbox security'}));
...
async function saveSettings(){
  ...
  if(on('s-sb-on')!==undefined)patch.sandbox={enabled:on('s-sb-on'),root:(val('s-sb-root')||'').trim(),
    folders:(val('s-sb-folders')||'').split('\n').map(s=>s.trim()).filter(Boolean)};
