...
var JOBS={recipes:[],personas:[],persona:'',deliveries:[],installed:[],summary:null,ready:null,accounts:{},pick:'',busy:false,tab:'mine'};
var JOB_TABS=['mine','build'];
/* Mine = the value view; Build = the flows editor that used to be the Workflows app
...
function jobTabs(){
  return `<div class="job-tabs">${segTabs('job-tabs',['Mine','Build'],Math.max(0,JOB_TABS.indexOf(JOBS.tab)),'jobSetTab')}</div>`;
}
function jobSetTab(i){JOBS.tab=JOB_TABS[i]||'mine';refreshApp('jobs')}
