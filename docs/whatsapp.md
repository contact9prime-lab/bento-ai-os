...
| | **WhatsApp Web link** | **Business (Cloud) API** (default) |
|---|---|---|
...
Linking from Settings → Channels or with `--pair` switches the mode for you — choosing to link IS choosing the transport, so there is no dropdown to set first.
...
It needs **Node.js**; the bridge is a small Node program that ships with AgentOS
(`agentos/wa_bridge/`) and fetches its own dependencies on request, because there
is no maintained Python client for the protocol. Install it from the
WhatsApp card, or:

```
bento channels whatsapp --set mode=baileys
bento channels whatsapp --pair               # draws the QR in the terminal
bento channels whatsapp --unpair             # forget the credentials
```
...
**Unlinking deletes the credentials.** `~/.agentos/whatsapp/session`
