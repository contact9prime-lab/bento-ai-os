...
> **说明：** 本翻译由 AI 生成，尚未经人工校对，可能存在错误。如有出入，以[英文原版](../../README.md)为准。
...
以前，桌面一打开就是 43 个图标，而这台机器连一个问题都还答不上来。现在：

![第一天的桌面：三个分组展开，三个折叠为带计数的一行，菜单栏里有"还没有大脑 · 给它一个"的提示](../design/ux-review/after/a-desktop.jpg)

- **Deck 会折叠你还没用过的分组。** Intelligence、System 和 Library 起初只是带计数和图标预览的一行；点一下就展开，或者在你第一次打开其中某个应用时自动展开。你自己整理过的 deck 会原样保留。
- **菜单栏会说明机器处于什么状态。** 当没有任何东西可以作答时，它显示 *还没有大脑 · 给它一个 →*，点击即打开安装向导里对应的那一步。
- **失败的一轮对话是一句话加一扇门。** 没有模型、Ollama 没运行、密钥被拒、触发速率限制、模型没有下载——每一种都是一句话和一个按钮（*给它一个大脑*、*打开 AI 提供商*、*打开 Model Manager*），出现在聊天、提示栏的卡片和每个副驾驶面板里。异常的名字进入日志，那才是它该在的地方。
- **所有弹出层遵循同一条规则。** 启动器、菜单、通知、电源和快速设置一次只打开一个，按 Escape 或点击外部即关闭，启动器也能完整显示在当前屏幕上。
- **在手机上只有一个输入框。** 带有自己输入框的面板会隐藏提示栏；列表/详情类应用一次只显示一栏并提供返回；安装向导保留了退出的入口。

![全新安装后的第一条消息："现在还没有东西能回答——这台机器没有大脑"，并附有"给它一个大脑"按钮，而不是一条 Python 异常](../design/ux-review/after/a-chat-sent.jpg)

以上每一项都在真实浏览器中做了前后测量。发现这些问题的评审及其数据见 [docs/design/ux-review-2026-09.md](../design/ux-review-2026-09.md)。
...
然后打开 **http://127.0.0.1:8321**，或者运行 `bento setup` 在终端里走一遍同样的十二个步骤。
