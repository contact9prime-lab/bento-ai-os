...
![运转中的 Bento Box AI —— 向智能体索要一份晨间简报并得到实时回复、堆叠窗口，以及在五种设计语言之间切换](../screenshots/demo.gif)

<sub>▶ [观看完整片段（MP4）](../screenshots/demo.mp4) —— 一次在设备上真实作答的对话轮次；上方的聊天回复是实时智能体，而非演示假象。</sub>
...
![首次运行的安装界面：左侧是九个步骤的导轨，右侧是"为你的智能体命名"，附带一行字"你最终会得到：菜单栏上以及每条回复里的那个名字"](../screenshots/onboarding-1-name.png)
...
![窗口中的 Setup 应用：左侧是九步导轨，右侧打开的是"构建一名专家"步骤](../screenshots/setup-app.png)
...
然后打开 **http://127.0.0.1:8321**，或者运行 `bento setup` 在终端里走一遍同样的九个步骤。

它会在你的 `PATH` 上留下一个 `bento` 命令（在 `~/.local/bin` 中，如果那里原本没有，就会添加到你的 shell 配置文件里——之后请打开一个新终端）。
...
| 参数 | 它的作用 |
|---|---|
| `--passphrase=SECRET` | 登录时要求提供它，并允许绑定到回环之外 |
...
```bash
bento remote --on --passphrase 'something long' --bind 0.0.0.0   # 地址
bento remote --port 8080                                          # 端口
bento remote                                                      # 现在是什么状态
```
...
> **能被其他机器访问是一个有意的选择，不是默认设置。** Bento 仅监听 `127.0.0.1`，直到你给它一个口令为止，因为智能体拥有一个真实的 shell——这里的一个开放端口就是一个开放的 shell。单用 `--bind` 会因此被拒绝，`bento serve --host 0.0.0.0` 在远程访问关闭时同样会被拒绝。
