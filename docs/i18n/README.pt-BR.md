...
> **Aviso:** esta tradução foi gerada por IA e ainda não foi revisada por uma pessoa; pode conter erros. Em caso de divergência, vale a [versão em inglês](../../README.md).
...
## O primeiro dia pesa menos que a quarta semana

A área de trabalho abria com 43 ícones em uma máquina que ainda não conseguia responder a uma pergunta. Agora:

![A área de trabalho no primeiro dia: três grupos abertos, três dobrados em uma linha com contador, e na barra de menus o chip "Ainda sem cérebro · dê um a ela"](../design/ux-review/after/a-desktop.jpg)

- **O deck dobra o que você ainda não usou.** Intelligence, System e Library começam como uma linha com contador e uma prévia dos ícones; desdobram com um clique, ou na primeira vez que você abre um de seus apps. Um deck que você organizou fica exatamente como você deixou.
- **A barra de menus diz em que estado a máquina está.** Sem nada configurado para responder, ela mostra *Ainda sem cérebro · dê um a ela →*, e clicar abre essa etapa da configuração.
- **Um turno que falhou é uma frase com uma porta.** Sem modelo, Ollama parado, uma chave recusada, um limite de requisições, um modelo não baixado: cada um é uma frase e um botão (*Dê um cérebro a ela*, *Abrir provedores de IA*, *Abrir Model Manager*), no Chat, no cartão da barra de entrada e em cada painel copiloto. O nome da exceção vai para os Logs, que é o lugar dele.
- **Uma regra para cada popover.** O lançador, os menus, as notificações, energia e Ajustes rápidos abrem um de cada vez, fecham com Esc e com um clique fora, e o lançador cabe na tela em que está.
- **No celular, uma única caixa de escrita.** Uma folha com entrada própria esconde a barra de entrada; apps de lista/detalhe mostram um painel por vez com caminho de volta; o assistente de configuração mantém sua saída.

![A primeira mensagem em uma instalação nova: "Nada pode responder ainda — esta máquina não tem cérebro" com um botão "Dê um cérebro a ela", em vez de uma exceção do Python](../design/ux-review/after/a-chat-sent.jpg)

Cada um desses pontos foi medido em um navegador real antes e depois. A revisão que os encontrou, com os números: [docs/design/ux-review-2026-09.md](../design/ux-review-2026-09.md).
