...
![O Bento Box AI em movimento — pedindo ao agente um resumo matinal e recebendo uma resposta ao vivo, empilhando janelas e alternando entre cinco linguagens de design](../screenshots/demo.gif)

<sub>▶ [Assista ao clipe completo (MP4)](../screenshots/demo.mp4) — um turno real respondido no próprio dispositivo; a resposta do chat acima é o agente ao vivo, não uma simulação.</sub>
...
estivesse lá — abra um novo terminal depois).
...
| flag | o que ela faz |
|---|---|
| `--passphrase=SECRET` | exige isto para entrar, e permite vincular fora do loopback |
...
| `--yes` | responde sim a todo componente opcional |
...
```bash
bento remote --on --passphrase 'something long' --bind 0.0.0.0   # o endereço
bento remote --port 8080                                          # a porta
bento remote                                                      # o que é agora
```
...
> `127.0.0.1` apenas até você dar uma frase-senha, porque o agente tem um shell de verdade — uma porta aberta
> aqui é um shell aberto. `--bind` sozinho é recusado por esse motivo, e também
> `bento serve --host 0.0.0.0` com o acesso remoto desligado.
