...
![Bento Box AI in Bewegung — den Agenten um ein Morgen-Briefing bitten und eine Live-Antwort erhalten, Fenster stapeln und zwischen fünf Designsprachen wechseln](../screenshots/demo.gif)

<sub>▶ [Den vollständigen Clip ansehen (MP4)](../screenshots/demo.mp4) — ein echter, auf dem Gerät beantworteter Zug; die Chat-Antwort oben ist der Live-Agent, kein Mockup.</sub>
...
nicht schon dort war — öffnen Sie danach ein neues Terminal).
...
| Flag | Was es tut |
|---|---|
| `--passphrase=SECRET` | dies zur Anmeldung verlangen und das Binden abseits von Loopback erlauben |
...
| `--yes` | zu jeder optionalen Komponente Ja sagen |
...
> `127.0.0.1`, bis Sie ihm eine Passphrase geben, denn der Agent hat eine echte Shell — ein offener
> Port hier ist eine offene Shell. `--bind` allein wird aus diesem Grund abgelehnt, ebenso wie
> `bento serve --host 0.0.0.0` bei ausgeschaltetem Fernzugriff.
