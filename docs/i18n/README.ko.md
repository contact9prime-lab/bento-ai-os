...
> **안내:** 이 번역은 AI가 생성한 것으로 아직 사람이 검수하지 않았으며 오류가 있을 수 있습니다. 내용이 다를 경우 [영어 원문](../../README.md)이 우선합니다.
...
예전에는 아직 질문에 답하지 못하는 기계 위에 데스크톱이 43개의 타일을 펼쳐 놓고 열렸습니다. 이제는:

![첫날의 데스크톱: 세 그룹은 펼쳐지고, 세 그룹은 개수가 표시된 한 줄로 접혀 있으며, 메뉴 바에 "아직 두뇌가 없습니다 · 하나 주세요" 칩이 있음](../design/ux-review/after/a-desktop.jpg)

- **덱은 아직 쓰지 않은 것을 접어 둡니다.** Intelligence, System, Library는 개수와 아이콘 미리보기가 있는 한 줄로 시작하고, 클릭하거나 그 그룹의 앱을 처음 열 때 펼쳐집니다. 직접 정리한 덱은 두고 간 그대로 남습니다.
- **메뉴 바가 기계의 상태를 말해 줍니다.** 답할 것이 아무것도 설정되지 않았으면 *아직 두뇌가 없습니다 · 하나 주세요 →* 라고 표시되고, 클릭하면 설정의 바로 그 단계가 열립니다.
- **실패한 턴은 문이 달린 한 문장입니다.** 모델 없음, Ollama 미실행, 거부된 키, 속도 제한, 내려받지 않은 모델 — 각각이 한 문장과 버튼 하나(*두뇌 주기*, *AI 제공자 열기*, *Model Manager 열기*)가 되어 채팅, 프롬프트 바의 카드, 모든 코파일럿 패널에 나타납니다. 예외의 이름은 있어야 할 곳인 로그로 갑니다.
- **모든 팝오버에 하나의 규칙.** 런처, 메뉴, 알림, 전원, 빠른 설정은 한 번에 하나만 열리고, Escape와 바깥 클릭으로 닫히며, 런처는 지금 보고 있는 화면 안에 들어옵니다.
- **휴대전화에서는 입력창 하나.** 자체 입력창이 있는 시트는 프롬프트 바를 숨기고, 목록/상세형 앱은 돌아갈 길과 함께 한 번에 한 면만 보여 주며, 설정 마법사는 나가는 문을 유지합니다.

![새 설치에서의 첫 메시지: Python 예외 대신 "아직 아무것도 답할 수 없습니다 — 이 기계에는 두뇌가 없습니다"와 "두뇌 주기" 버튼](../design/ux-review/after/a-chat-sent.jpg)

이 모든 항목은 실제 브라우저에서 전후를 측정했습니다. 이를 찾아낸 리뷰와 수치는 [docs/design/ux-review-2026-09.md](../design/ux-review-2026-09.md)에 있습니다.
