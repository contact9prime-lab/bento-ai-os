...
![Bento Box AI en action — on demande à l'agent un briefing matinal et on obtient une réponse en direct, on empile des fenêtres et on bascule entre cinq langages de design](../screenshots/demo.gif)

<sub>▶ [Regarder le clip complet (MP4)](../screenshots/demo.mp4) — un vrai tour de conversation traité sur l'appareil ; la réponse du chat ci-dessus est l'agent en direct, pas une maquette.</sub>
...
pas — ouvrez un nouveau terminal ensuite).
...
|---|---|
| `--passphrase=SECRET` | l'exiger pour se connecter, et autoriser la liaison hors loopback |
...
| `--yes` | répondre oui à chaque composant optionnel |
...
```bash
bento remote --on --passphrase 'something long' --bind 0.0.0.0   # l'adresse
bento remote --port 8080                                          # le port
bento remote                                                      # ce que c'est maintenant
```
...
> `127.0.0.1` jusqu'à ce que vous lui donniez une phrase de passe, parce que l'agent a un vrai shell — un port ouvert
> ici est un shell ouvert. `--bind` seul est refusé pour cette raison, tout comme `bento serve --host 0.0.0.0` avec
> l'accès à distance désactivé.
