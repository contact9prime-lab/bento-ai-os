...
> **تنبيه:** هذه الترجمة أُنشئت بواسطة الذكاء الاصطناعي ولم يراجعها بشر بعد، وقد تحتوي على أخطاء. عند الاختلاف، تكون [النسخة الإنجليزية](../../README.md) هي المعتمدة.
...
## الإعداد اثنتا عشرة خطوة، وكل خطوة تترك أثرًا خلفها
...
![شاشة الإعداد عند التشغيل الأول: شريط من إحدى عشرة خطوة على اليسار، وعلى اليمين "سمِّ وكيلك" مع سطر "ستحصل في النهاية على: الاسم على شريط القوائم وفي كل رد"](../screenshots/onboarding-1-name.png)
...
![تطبيق الإعداد في نافذة: شريط الخطوات الإحدى عشرة على اليسار، وخطوة "ابنِ متخصصًا" مفتوحة على اليمين](../screenshots/setup-app.png)
...
## اليوم الأول أخفّ من الأسبوع الرابع

كان سطح المكتب يُفتح على 43 بلاطة في جهاز لا يستطيع بعد الإجابة عن سؤال واحد. الآن:

![سطح المكتب في اليوم الأول: ثلاث مجموعات مفتوحة، وثلاث مطويّة في سطر واحد مع عدّاد، وفي شريط القوائم شارة «لا دماغ بعد · أعطه واحدًا»](../design/ux-review/after/a-desktop.jpg)

- **يطوي الرفّ ما لم تستخدمه بعد.** تبدأ Intelligence وSystem وLibrary كسطر واحد مع عدّاد ولمحة من أيقوناتها؛ وتنفتح بنقرة، أو في أول مرة تفتح فيها أحد تطبيقاتها. الرفّ الذي رتّبته بنفسك يبقى كما تركته تمامًا.
- **يقول شريط القوائم في أي حال الجهاز.** إن لم يكن هناك ما يجيب، يظهر فيه *لا دماغ بعد · أعطه واحدًا ←*، والنقر عليه يفتح تلك الخطوة من الإعداد.
- **الدور الفاشل جملة لها باب.** لا نموذج، أو Ollama لا يعمل، أو مفتاح مرفوض، أو حدّ للمعدّل، أو نموذج لم يُنزَّل — كلٌّ منها جملة واحدة وزر واحد (*أعطه دماغًا*، *افتح مزوّدي الذكاء الاصطناعي*، *افتح Model Manager*)، في الدردشة، وفي بطاقة شريط الطلب، وفي كل لوحة مساعد. أما اسم الاستثناء فيذهب إلى السجلات، حيث مكانه.
- **قاعدة واحدة لكل نافذة منبثقة.** المشغّل والقوائم والإشعارات والطاقة والإعدادات السريعة تُفتح واحدة في كل مرة، وتُغلق بـ Escape وبالنقر خارجها، ويتّسع المشغّل للشاشة التي هو عليها.
- **على الهاتف، مربّع كتابة واحد.** الورقة التي لها مدخلها الخاص تُخفي شريط الطلب؛ وتطبيقات القائمة/التفاصيل تعرض لوحة واحدة في كل مرة مع طريق للعودة؛ ومعالج الإعداد يحتفظ بمخرجه.

![أول رسالة في تثبيت جديد: «لا شيء يستطيع الإجابة بعد — هذا الجهاز بلا دماغ» مع زر «أعطه دماغًا»، بدلًا من استثناء Python](../design/ux-review/after/a-chat-sent.jpg)

كل بند من هذه قِيس في متصفّح حقيقي قبل التغيير وبعده. المراجعة التي وجدتها، مع الأرقام، في [docs/design/ux-review-2026-09.md](../design/ux-review-2026-09.md).
...
ثم افتح **http://127.0.0.1:8321**، أو شغّل `bento setup` لنفس الخطوات الاثنتي عشرة في طرفية.

إن وُجدت طرفية يمكن السؤال عندها، يسأل المثبِّت سؤالين قبل أن ينتهي: هل ينبغي أن يكون هذا الجهاز قابلاً
للوصول من أجهزتك الأخرى، وإن كان كذلك، أتسجّل الدخول بـ**عبارة سر** أم بـ**حساب**. على جهاز بلا شاشة،
يكون السؤال الأول هو الفارق بين تثبيت يمكنك النظر إليه وتثبيت لا يمكنك. و`--yes` لا يجيب عنه عمداً:
منفذ مفتوح هنا هو صدفة مفتوحة.

يترك أمر `bento` على `PATH` لديك (في `~/.local/bin`، يُضاف إلى ملف تعريف الصدفة إذا لم
يكن هناك — افتح طرفية جديدة بعد ذلك). يعرض `bento --help` الأوامر العشرة التي يحتاجها جهاز جديد، وبقيتها في `bento help --all`.
...
| العَلَم | ما يفعله |
|---|---|
| `--passphrase=SECRET` | اطلب هذا لتسجيل الدخول، واسمح بالربط خارج loopback — إن أُعطي هنا فلن يسأل المثبِّت |
...
| `--yes` | أجب بنعم على كل مكوّن اختياري — و**ليس** على فتح المنفذ |
...
> `127.0.0.1` فقط حتى يصبح له قفل — عبارة سر، أو حساب يسجّل به أحدهم الدخول — لأن للوكيل صدفة حقيقية، ومنفذ
> مفتوح هنا هو صدفة مفتوحة. `--bind` وحده يُرفض لهذا السبب، وكذلك
> `bento serve --host 0.0.0.0` مع إيقاف الوصول عن بُعد. القفلان بديلان لا طبقتان: ما إن يوجد حساب حتى يصبح هو القفل، وعبارة السر أمامه إعداد لا يقرؤه شيء.
...
| `uv run bento user add <اسم>` | الحسابات — أوّل حساب يرث هذا الجهاز ويكون مسؤولاً |
| `uv run bento help --all` | كل الأوامر؛ و`bento --help` يعرض العشرة التي يحتاجها جهاز جديد |
...
- **Claude Code** أو **Gemini CLI** أو **Codex** — يمكن لوكيل ذكاء اصطناعي مثبّت هنا مسبقًا أن يكون العقل، مسجَّل الدخول بحسابك أنت؛ لا يمرّر له AgentOS أي مفتاح أبدًا.
