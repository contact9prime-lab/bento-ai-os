...
![La pantalla de configuración inicial: un riel de once pasos a la izquierda, y a la derecha "Ponle nombre a tu agente" con la línea "Acabarás con: el nombre en la barra de menú y en cada respuesta"](../screenshots/onboarding-1-name.png)
...
![La app de Configuración en una ventana: el riel de once pasos a la izquierda, y el paso "Construir un especialista" abierto a la derecha](../screenshots/setup-app.png)
...
**frase de contraseña** o con una **cuenta**. En una máquina sin pantalla, esa primera pregunta es la
diferencia entre una instalación que puedes mirar y una que no. `--yes` deliberadamente no la responde:
un puerto abierto aquí es una shell abierta.

Deja un comando `bento` en tu `PATH` (en `~/.local/bin`, añadido a tu perfil de shell si no estaba ahí —
abre una terminal nueva después). `bento --help` muestra los diez comandos que necesita una máquina nueva; `bento help --all` es el resto.
...
| flag | qué hace |
|---|---|
| `--passphrase=SECRET` | exigir esto para iniciar sesión, y permitir la vinculación fuera de loopback — dado aquí, el instalador no pregunta |
...
| `--yes` | responder sí a cada componente opcional — **no** a abrir el puerto |
...
> solo en `127.0.0.1` hasta que tiene un cerrojo — una frase de contraseña, o una cuenta con la que alguien
> inicia sesión —, porque el agente tiene una shell real y un
> puerto abierto aquí es una shell abierta. `--bind` por sí solo se rechaza por esa razón, y también
> `bento serve --host 0.0.0.0` con el acceso remoto desactivado. Los dos cerrojos son alternativas, no capas: en cuanto existe una cuenta, ella *es* el cerrojo, y una frase de contraseña delante es configuración que nada lee.
...
| `uv run bento user add <nombre>` | cuentas — la primera adopta esta máquina y es administradora |
| `uv run bento help --all` | todos los comandos; `bento --help` muestra los diez que necesita una máquina nueva |
